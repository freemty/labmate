...
for skill in "$ROOT"/skills/*/SKILL.md; do
  ...
  [ "$words" -le 500 ] || echo "REVIEW: $skill has $words words" >&2
  description=$(awk '
    /^description:/ {capture=1}
    capture {print}
    capture && /^---$/ {exit}
  ' "$skill")
  printf '%s' "$description" | grep -Eq '^description:[[:space:]]*[^[:space:]]' \
    || { echo "$skill description must declare a non-empty scalar or YAML block" >&2; exit 1; }
...
for agent in "$ROOT"/agents/*.md; do
  ...
  [ "$words" -le 350 ] || echo "REVIEW: $agent has $words words" >&2
done
...
[ "$session_chars" -le 512 ] \
  || echo "REVIEW: SessionStart output exceeds 512 chars" >&2
